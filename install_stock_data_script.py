...
def downloadStockData (ticker, start_date, end_date, directory):
    ...
    daily_file = os.path.join(directory, f"{ticker}_Day.csv")
    ...
    weekly_file = os.path.join(directory, f"{ticker}_Week.csv")
    ...
    weekly_data = weekly_data.sort_index(ascending=False)
    monthly_file = os.path.join(directory, f"{ticker}_Month.csv")
